...
class rndstr_structure:
    ...
    def from_file(self,file_name):
        ...
        self.scaling,self.lattice_vectors_scaled,self.basis_vectors_scaled,self.atom_strings_subl = read_atat_rndstr(self.file_N)
            
        self.num_atoms=len(self.atom_strings_subl)
        
        self.lattice_vectors = np.matmul(self.lattice_vectors_scaled,self.scaling)        
    ...
    def differentiate_subl(self):
        ...
        for c_i, i in enumerate(self.atom_strings_subl):
            for j in i:
                ...
                comps_of_site[c_i].append(float(j.split('=')[1].replace(',','')))
        all_atoms=list(set([item for sublist in atoms_of_site for item in sublist]))
    ...
    def rnds_add_unique(self,df_i):
        ...
        for i in range(len(df)):
            if df.loc[i,'unique']=='Y':
                ...
                if (df.loc[i,'Ni']+df.loc[i,'Cu'])>(df.loc[i,'Ti']+df.loc[i,'Hf']):
                    unique_label='Ni'
                else:
                    unique_label='Ti'
                for j in range(i+1,len(df)):
                    #hardcoded to our system
                    if np.array_equal(df.loc[i,self.all_atoms].astype(float), df.loc[j,self.all_atoms].astype(float)):
    ...
    def __repr__ (self):
        ...
        #r = json.dumps(self.__dict__,indent=2,cls=NpEncoder) json doesn't look that noce
        string_repr = ''
...
class supercell_structure:
    ...
    def from_file(self,file_name):
        ...
        with open(self.file_N, 'r') as f:
            self.rndstr_FC=f.read()
        self.scaling,self.lattice_vectors_scaled,self.basis_vectors_scaled,self.atoms = read_atat_sqs(self.file_N)
...
class sqs_one:
    ...
    def __init__(self,SQS_file_N,in_dir=True):
        ...
        if in_dir:
            ...
            self.digit_reference=re.sub('\D', '',self.SQS_file_PATH.split('/')[-1])
            
            self.clusters_PATH=self.SQS_dir_PATH+'/clusters.out'
            
            
            self.corr_PATH=self.SQS_dir_PATH+'/bestcorr-'+self.digit_reference+'.out'
    ...
    def get_mult_matrix(self):
        self.mult_matrix=(self.sup_obj.lattice_vectors@np.linalg.inv(self.rnd_obj.lattice_vectors)).round(2)
    ...
    def read_sqscellout(self):
        ...
        self.supercells=np.array([float(item) for sublist in [i.split() for i in buf] for item in sublist]).reshape(
        self.number_of_supercell,3,3)
        
        
    def create_periodic_supercell(self):
        #mod
        #print(self.rnd_obj.ase_struct)
        #print(self.mult_matrix)
        #X_test=self.mult_matrix.round(2)
        self.per_sup=make_supercell(self.rnd_obj.ase_struct,self.mult_matrix)
    ...
    def read_cluster(self):
        ...
        while ii < len(buf):
            ...
            arr_dum=np.array([float (j) for row in [i.split() for i in buf[ii:ii+int(point[-1])]] for j in row ]).reshape(
                int(point[-1]),5)
            ...
            good=ii+point[-1]
            ii=int(ii+point[-1])
        self.mult,self.diam,self.point,self.coors,self.spe2,self.clusf=mult,diam,point,coors,spe2,clusf
    ...
    def read_corr(self):
        ...
        self.c_best=buf[:,2] #the correlations of the best SQS found so far (3rd column)
        self.c_diso=buf[:,3] #along with the target disordered state correlation (4th column)
        self.c_diff=buf[:,4] #and the difference between the two (5th column)
...
def create_sqs_line(pwd_dir,dig):
    ...
    for file in os.listdir(pwd_dir):
        ...
        if filename.startswith('bestsqs') and filename.endswith(str(dig)+'.out'):
            fns.append(filename)
    num_sqs=len(fns)
    sqs_line=[]
